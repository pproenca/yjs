...
const emitTransactionUpdate = (transaction, doc) => {
  if (doc._observers.has('update')) {
    ...
    if (reservedRuntime === undefined) {
      ...
      const encoder = readReservedEmissionContexts(transaction).v1
      if (writeReservedMutationUpdate(encoder, transaction)) {
...
const emitTransactionUpdateV2 = (transaction, doc) => {
  if (doc._observers.has('updateV2')) {
    ...
    if (reservedRuntime === undefined) {
      ...
      const encoder = readReservedEmissionContexts(transaction).v2
      if (writeReservedMutationUpdate(encoder, transaction)) {
