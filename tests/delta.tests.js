...
const assertNamedFailure = (run, name) => {
  ...
  t.assert(failure instanceof Error && failure.name === name, `expected ${name}, got ${failure instanceof Error ? failure.name : 'no failure'}`)
}
...
/** @param {() => unknown} run @param {string} name */
const assertRecordedNamedFailure = (run, name) => {
  let failure = null
  try {
    run()
  } catch (error) {
  ...
  const failureName = failure instanceof Error ? failure.name : null
  t.assert(failureName === name, `expected ${name}, got ${failureName ?? 'no failure'}`)
}

export const testDeltaReservedMutationAuthority = () => {
  const doc = new Y.Doc()
  const type = doc.get('content')
  const discarded = type.reserveDeltaMutation(delta.create().insert('discarded').done())
  discarded.discard()
  assertRecordedNamedFailure(() => discarded.apply(), 'DeltaMutationCapabilityError')
  const forged = Object.create(Object.getPrototypeOf(discarded))
  assertRecordedNamedFailure(() => forged.apply(), 'DeltaMutationCapabilityError')

  const prepared = type.reserveDeltaMutation(delta.create().insert('accepted').done())
  prepared.apply()
  assertRecordedNamedFailure(() => prepared.apply(), 'DeltaMutationCapabilityError')
  t.assert(type.toString() === 'accepted')
}

export const testDeltaReservedMutationCapturesExactAtomicRanges = () => {
  const doc = new Y.Doc()
  doc.clientID = 11
  const type = doc.get('content')
  let updates = 0
  doc.on('update', () => { updates++ })
  const prepared = type.reserveDeltaMutation(
    delta.create().insert('abc').setAttr('claim', 'pending').done()
  )
  t.assert(updates === 0 && Object.isFrozen(prepared.reservation))
  let exact = null
  prepared.apply({
    afterMutation: content => {
      exact = content
      type.setAttr('receipt', 'committed')
    }
  })
  const captured = /** @type {import('../src/utils/delta-mutation.js').DeltaMutationContent} */ (/** @type {unknown} */ (exact))
  t.assert(updates === 1 && type.getAttr('receipt') === 'committed')
  t.assert(exact !== null && Object.isFrozen(captured) && Object.isFrozen(captured.inserts) && Object.isFrozen(captured.deletes))
  t.assert(captured.rangeCount === captured.inserts.length + captured.deletes.length)
  t.assert(captured.rangeCount <= prepared.reservation.rangeCountUpperBound)
  t.assert(captured.inserts.length === 1 && captured.inserts[0].client === 11 && captured.inserts[0].clock === 0)
  t.assert(captured.inserts[0].length + 1 === doc.store.getClock(11), 'callback allocation is excluded')
}

export const testDeltaReservedMutationUsesStructuralCapacity = () => {
  const text = 'x'.repeat(100000)
  const insertDoc = new Y.Doc()
  const insert = insertDoc.get('content').reserveDeltaMutation(delta.create().insert(text).done())
  let exact = null
  insert.apply({ afterMutation: content => { exact = content } })
  const capturedInsert = /** @type {import('../src/utils/delta-mutation.js').DeltaMutationContent} */ (/** @type {unknown} */ (exact))
  t.assert(insert.reservation.rangeCountUpperBound === 1)
  t.assert(exact !== null && capturedInsert.inserts.length === 1 && capturedInsert.inserts[0].length === text.length)

  const deleteDoc = new Y.Doc()
  const deleteType = deleteDoc.get('content')
  deleteType.applyDelta(delta.create().insert(text).done())
  const deletion = deleteType.reserveDeltaMutation(delta.create().delete(text.length).done())
  deletion.apply({ afterMutation: content => { exact = content } })
  const capturedDelete = /** @type {import('../src/utils/delta-mutation.js').DeltaMutationContent} */ (/** @type {unknown} */ (exact))
  t.assert(deletion.reservation.rangeCountUpperBound === 1)
  t.assert(exact !== null && capturedDelete.deletes.length === 1 && capturedDelete.deletes[0].length === text.length)
}

export const testDeltaReservedMutationRejectsWireBoundaryBeforeMutation = () => {
  const doc = new Y.Doc()
  const type = doc.get('content')
  const before = Y.encodeStateAsUpdate(doc)
  assertRecordedNamedFailure(
    () => type.reserveDeltaMutation(delta.create().insert('x'.repeat(200000)).done()),
    'DeltaMutationPreparationError'
  )
  const after = Y.encodeStateAsUpdate(doc)
  t.assert(before.length === after.length && before.every((value, index) => value === after[index]))
}
